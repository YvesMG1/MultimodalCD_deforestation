...
class ConvTransformerTokensToEmbeddingNeck(nn.Module):
    ...
    def __init__(
        ...
        # Calculate the number of stages needed to reach the target dimensions
        num_stages_H = int(np.log2(self.target_H / self.Hp))
        num_stages_W = int(np.log2(self.target_W / self.Wp))
        num_stages = max(num_stages_H, num_stages_W)

        self.H_out = Hp
        self.W_out = Wp
        for _ in range(num_stages):
            self.H_out = _convTranspose2dOutput(self.H_out, stride, padding, dilation, kernel_size, output_padding)
            self.W_out = _convTranspose2dOutput(self.W_out, stride, padding, dilation, kernel_size, output_padding)

        self.fpn1 = nn.Sequential(
            nn.ConvTranspose2d(
                self.embed_dim,
                self.output_embed_dim // 2,
                kernel_size=kernel_size,
                stride=stride,
                dilation=dilation,
                padding=padding,
            ...
            Norm2d(self.output_embed_dim // 2),
            nn.GELU(),
            nn.ConvTranspose2d(
                self.output_embed_dim // 2,
                self.output_embed_dim,
